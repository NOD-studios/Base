...
System.register(['./index'], function (_export, _context) {
  var Base, Configuration, _createClass, makeErrorMessage, ExampleConfiguration, Example, example, errorMessage;
  ...
  return {
    ...
    execute: function () {
      ...
      makeErrorMessage = function makeErrorMessage(name, property) {
      ...
      example = new Example(new ExampleConfiguration({
        secondVal: 'test'
      }));

      if (example.initPassed !== true) {
        errorMessage = makeErrorMessage(example.constructor.name, 'initPassed');
        throw new Error(errorMessage);
      }
    }
  };
